...
const EnquirePopup = ({ isOpen, onClose, yachtName }) => {
  ...
  return (
    <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-[#0D1B2A]/80 backdrop-blur-sm transition-opacity duration-300">
      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-transparent rounded-2xl overflow-hidden shadow-2xl">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-50 p-3 bg-[#0D1B2A]/50 rounded-full text-white hover:bg-white hover:text-black transition-colors duration-200"
        >
          <X size={24} />
        </button>
        ...
        <div className="absolute top-0 left-0 w-full p-4 z-40 pointer-events-none">
          <p className="text-white/80 text-xs font-bold uppercase tracking-widest text-center mt-2">
            Enquiring about: <span className="text-[#C9A84C]">{yachtName}</span>
          </p>
        </div>
