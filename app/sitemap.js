...
export default async function sitemap() {
  ...
  //
  // 2026-08-13. Everything before this line assigns lastmod by content FAMILY,
  // and each of those dates was honest when it was written. The result was
  // not: 469 of 476 URLs published the identical 2026-08-08, because a family
  // covers dozens of pages and any edit inside it moves all of them. A sitemap
  // that says "everything changed at once" cannot answer the one question a
  // crawler asks it, and Google's guidance is explicit that it uses lastmod
  // only while it stays accurate.
  //
  // What that cost, measured the same day with the URL Inspection API: 44 of
  // 476 pages re-crawled since 6 August, thirteen of our twenty biggest pages
  // last fetched in June or July, and 73 URLs outside the index altogether.
  // The copy fixes of the last week are live and unseen.
  //
  // lib/lastmodManifest.json carries a content hash and a date per URL,
  // regenerated by scripts/freshness.mjs against the built HTML. A page keeps
  // its date until its rendered text actually changes. Family dates stay as
