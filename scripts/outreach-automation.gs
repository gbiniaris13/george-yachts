// =====================================================
// GEORGE YACHTS — OUTREACH AUTOMATION v2
...
function sendOutreach(skipHoursCheck) {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    try {
      ...
      if (status === 'new' || status === '') {
        ...
        // Telegram notification with LinkedIn link + copy-paste message
        if (linkedinUrl) {
          sendTelegram_(
            '\ud83d\udce7 *Email 1 sent* to ' + firstName + ' (' + company + ')\n' +
            '\ud83c\udf0d ' + getTimezoneForCountry_(country) + '\n\n' +
            '\ud83d\udd17 *LinkedIn:* ' + linkedinUrl + '\n\n' +
            '\ud83d\udccb *Copy-paste for LinkedIn:*\n' +
            '_Hi ' + firstName + ', I just reached out via email about yacht charter partnerships in Greece. Would love to connect!_'
          );
        } else {
          sendTelegram_('\ud83d\udce7 *Email 1 sent* to ' + firstName + ' (' + company + ') \u2014 ' + email + '\n\ud83c\udf0d ' + getTimezoneForCountry_(country));
        }
      ...
      // --- FOLLOW-UP 1: 4 days after Email 1 ---
        ...
        if (daysSinceE1 >= CONFIG.FOLLOWUP_1_DAYS) {
          ...
          if (linkedinUrl) {
            sendTelegram_(
              '\ud83d\udd14 *Follow-up 1 about to send* to ' + firstName + ' (' + company + ')\n\n' +
              '\ud83d\udd17 *Approach on LinkedIn first:* ' + linkedinUrl + '\n\n' +
              '\ud83d\udccb *Copy-paste:*\n' +
              '_Hi ' + firstName + ', following up on my email about yacht charter partnerships in Greece. I\'d love to discuss how we can work together. Are you open to a quick chat?_'
            );
          }
      ...
      // --- FOLLOW-UP 2: 10 days after Follow-up 1 ---
        ...
        if (daysSinceFU1 >= CONFIG.FOLLOWUP_2_DAYS) {
          ...
          if (linkedinUrl) {
            sendTelegram_(
              '\ud83d\udd14 *Final follow-up about to send* to ' + firstName + ' (' + company + ')\n\n' +
              '\ud83d\udd17 *Last chance LinkedIn touch:* ' + linkedinUrl + '\n\n' +
              '\ud83d\udccb *Copy-paste:*\n' +
              '_Hi ' + firstName + ', I\'ve been trying to connect about Greek yacht charters. This is my final outreach \u2014 if the timing isn\'t right now, no worries at all. Wishing you a great season!_'
            );
          }
  ...
  if (remaining === 0 && sentThisRun > 0) {
    sendTelegram_(
      ...
      'Send me a CSV file with new leads to continue.\n\n' +
      'CSV format: first\\_name, last\\_name, email, company, country, linkedin\\_url'
    );
    ...
    if (!PropertiesService.getScriptProperties().getProperty(notifiedKey)) {
      sendTelegram_(
        '\ud83d\udcea *Pipeline empty* \u2014 no new leads to send.\n' +
...
function sendDailySummary() {
  ...
  var text = '\ud83d\udcca *Outreach Daily Report*\n' +
    ...
    '\ud83d\udce8 *Sent today: ' + getSentToday_() + ' emails*\n\n';
