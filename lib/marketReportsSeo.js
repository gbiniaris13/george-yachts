...
const INDEX_BANDS = {
  ...
  // 2026-09-14: read from the Index itself. This table had been typed by hand
  // and drifted: on 14 September it carried the Q4 bands next to the August
  // counts and was missing the two motor bands added on 11 September.
  rows: CHARTER_INDEX_2026.summaryTable.rows.map(({ cells }) => [
    cells[0], cells[1], cells[2].replace("-", " to "), cells[3],
  ]),
};
...
export const MARKET_REPORTS = [
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    methodology:
      "This retrospective was rewritten on 6 September 2026 to remove modelled figures that appeared in the April edition. Every number that remains is either taken from the George Yachts Greek Charter Index 2026, which compiles the current rate cards of the one hundred and five fully crewed yachts we represent, or from a third-party publication linked where the figure appears. Availability and demand statements are the observation of one Athens desk, not a survey of the Greek fleet, and the section marked opinion is exactly that. We do not hold industry-wide booking data and do not claim to.",
    faq: [
      {
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    sections: [
      ...
      },
      {
        ...
        body:
          `Peak pricing in Greece is the rate card itself: an owner quotes one Greece-wide weekly fee per yacht, and the shoulder discount is taken off that. The bands below are the current cards of the one hundred and five fully crewed yachts on the ${SRC.index}, per yacht per week before VAT and APA. VAT is invoiced at 5.2, 6.5, 7.8 or 12% by certification, APA runs 20 to 30% of the base on sailing yachts and catamarans and 30 to 40% on motor yachts, and a 10 to 15% crew gratuity on the base is customary.`,
        dataTable: INDEX_BANDS,
      },
      {
