...
export const kvExpire = (key, seconds) => exec(['EXPIRE', key, String(seconds)]);
export const kvLtrim = (key, start, stop) => exec(['LTRIM', key, String(start), String(stop)]);
...
// Set operations (for unique-member lists like newsletter subscribers)
export const kvSadd = (key, value) => exec(['SADD', key, value]);
export const kvSmembers = (key) => exec(['SMEMBERS', key]);
export const kvSrem = (key, value) => exec(['SREM', key, value]);
export const kvScard = (key) => exec(['SCARD', key]);
export const kvSismember = (key, value) => exec(['SISMEMBER', key, value]);
