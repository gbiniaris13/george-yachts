...
export const organizationSchema = {
  ...
  // 2026-08-07 — the catamaran correction, and it came from asking the engines
  // directly rather than guessing. Perplexity describes us accurately in every
  // other respect (Wyoming LLC, Athens, George P. Biniaris, MYBA, VAT, APA)
  // and then says we specialise in "luxury motor-yacht and superyacht
  // holidays". We are absent from every "best catamaran brokers in Greece"
  // answer on both ChatGPT and Perplexity.
  //
  // The reason was here. This description and knowsAbout below are the
  // machine-readable summary of who we are, and neither contained the word
  // catamaran, while 49 of the 72 yachts we place are catamarans and both
  // charters closed this season were catamarans. The homepage says it eleven
  // times; the structured data an engine actually trusts said it never.
  "description": "Boutique brokerage specialising in weekly fully crewed charters in Greek waters, on motor yachts, power catamarans and sailing catamarans. The Cyclades and the Saronic from Athens, the Ionian from Lefkada or Corfu; not the Dodecanese, not the Sporades. A curated crewed fleet placed personally. Weeks rather than days, starting on any day of the week, never day hire. MYBA-standard contracts, IYBA member.",
  ...
  // 2026-08-22, τα κενά που έβγαλε ο έλεγχος οντότητας. Μια μηχανή που
  // απαντά "ποιον broker να πάρω στην Ελλάδα" θέλει εικόνα, τρόπο επαφής,
  // νόμισμα και ώρες. Κάθε ένα από αυτά έλειπε.
  "image": "https://georgeyachts.com/images/gy-logo-mark-2x.png",
  "email": "george@georgeyachts.com",
  "currenciesAccepted": "EUR",
  // Ο ίδιος απαντά, οπότε οι ώρες είναι η αλήθεια της δουλειάς και όχι
  // ωράριο καταστήματος. Επτά μέρες μέσα στη σεζόν.
  ...
  "founder": {
    ...
    "knowsAbout": [
      ...
      "Weekly Crewed Yacht Charter",
      "Luxury Yacht Charter",
      "Greek Waters Navigation",
      "MYBA Charter Contracts",
      "Greek Charter VAT and APA",
      "Yacht Charter Broker Verification",
  ...
  // service is delivered in Greek waters (areaServed above); the people
  // it is built for are American. Search Console puts the United States
  // first among our countries, and every charter closed this season was
  // booked by American guests. A US company, a US line, an American
  // audience: stated where an engine reads it.
  // 2026-09-10 — the audience block that stood here moved to the umbrella
  // Service in lib/serviceSchema.js. schema.org does not define `audience`
  // on Organization, LocalBusiness or TravelAgency, only on CreativeWork,
  // Event, Product, Service and a few others, and Ahrefs was reporting the
  // node invalid on 607 pages, which is every page that carries it. The
  // fact is unchanged and still stated: the guests this house is built for
  // are American, now said on the Service, where the vocabulary allows it.
  "memberOf": {
    "@type": "Organization",
    ...
    "itemListElement": [
      {
        ...
        "itemOffered": {
          "@type": "Service",
          "name": "Motor Yacht Fleet - full-crew crewed yacht charter",
      ...
      },
      {
        ...
        "itemOffered": {
          "@type": "Service",
          "name": "Catamaran Fleet - crewed yacht charter",
          "serviceType": "Crewed yacht charter, per yacht per week"
        },
        "price": "11500",
        "priceCurrency": "EUR",
        "areaServed": "GR"
  ...
  //
  // 2026-08-07 — reordered and split, after asking the engines directly.
  // "Motor yacht charter Greece" sat second while catamarans were bundled
  // into one blurred entry in fifth place, "Catamaran and sailing yacht
  // charter Greece". Perplexity duly reports that we specialise in motor
  // yachts and superyachts, and we appear in nobody's catamaran list.
  //
  // The fleet: 17 motor yachts, 15 power catamarans, 27 sailing catamarans,
  // 6 monohulls. Every type is now its own entity rather than a blur.
  // Corrected 2026-08-22: this list still described the pre-withdrawal
  // fleet of 72 and said 5 monohulls where there are 6. The numbers now
  // come from lib/fleetCount.js FLEET_COMPOSITION, measured against the
  // live fleet API rather than typed by hand.
  //
  // Order corrected the same day on George's instruction. The first pass put
  // catamarans first because they are 49 of the 72 hulls, which was reasoning
  // from the fleet rather than from the business. His specialism, in his own
  // words, is "weekly crewed charters on motor yachts, power catamarans and
  // catamarans", in that order: motor yachts carry the higher weekly rates and
  // he is deliberately growing towards larger boats. The catamarans closed the
  // first two charters of the season and stay prominent, second and third.
  // Fleet count is not commercial priority, and only he knows the second.
  "knowsAbout": [
    "Weekly crewed motor yacht charter Greece",
    ...
    "Verifying a yacht charter broker",
    "UHNW client services"
  ],
