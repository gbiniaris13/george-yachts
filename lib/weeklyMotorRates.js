...
// 2026-08-08 — this page is entirely motor yachts and superyachts, and the
// APA on those runs 30-40% because fuel dominates the account (lib/apaFacts.js).
// It used to print the site-wide fallback, which understated every band here.
// Read from the same table the estimator charges with, so the sentence and the
// arithmetic cannot drift apart.
export const APA_PCT = Math.round(apaRateFor("motor-yacht") * 100);
...
export function buildMatrix() {
  return {
    caption:
      `Indicative all-in weekly cost (est.), crewed motor yacht, Greek waters. All-in = base charter fee + APA provisioning (${APA_PCT}%) + VAT estimated at the ${VAT_PCT}% statutory ceiling (most yachts invoice 5.2-12% by certification) + suggested crew gratuity (${GRATUITY_PCT}%). Each band shown at a representative length and build tier.`,
...
export function buildBreakdownTable() {
  ...
  return {
    ...
    rows: [
      ...
      { cells: [`VAT (est. at the ${VAT_PCT}% ceiling)`, euro(e.vat)] },
...
export function quickAnswer() {
  ...
  return (
    ...
    `All-in covers the base fee, APA provisioning (${APA_PCT}%), VAT at the ${VAT_PCT}% statutory ceiling (certified yachts invoice less) and crew gratuity (${GRATUITY_PCT}%). ` +
    `A typical 30m motor yacht runs about ${euroK(e.total)} all-in for a peak-season week.`
  );
}
...
export function methodology() {
  return (
    ...
    `VAT follows the 2026 Greek regime: a reduced ${VAT_PCT}% on commercial charters over 48 hours (every weekly charter qualifies); short charters under 48 hours, static charters, or bareboat charters without crew are taxed at the standard 24%. ` +
...
export function statCallouts() {
  ...
  return [
    ...
    { value: euroK(e.total), label: "All-in, 30m motor yacht, peak-season week" },
  ];
}
...
export function faqItems() {
  ...
  return [
    ...
    // The hospitality decade is the part no competing broker can claim.
    {
      ...
      answer:
        ...
        "Nothing on this list is a boat we are paid to fill. And George is a licensed sailing skipper through the Olympiacos SFP Sailing Academy with a powerboat licence valid to 25 metres, who ran charter seasons out of Corfu and operations across the Ionian, Cyclades and Saronic, before which he spent a decade in luxury hospitality operations with the Interni Group in Mykonos, at The Wild Hotel and Ftelia Beach Club, and founded two premium venues in Kifisia. " +
        "That combination is the whole proposition: someone who knows what the sea does to a schedule and what five-star service actually costs to deliver on a boat. Verifiable in a minute: IYBA Charter Active Member in their public directory, MYBA-standard contracts with every layer itemised before signature, George Yachts Brokerage House LLC of Wyoming operating from Athens, Forbes May 2026. Weekly and fully crewed only, though the week can start on any day rather than only the Saturday the market defaults to.",
    },
    {
    ...
    // 2026-08-01 SD-1 (GSC striking-distance): "motor yacht charter greece
    // prices" sat at pos 11.7 with 68 impressions and 0 clicks in July, and
    // the page never used that exact phrase - searchers say "prices", the
    // page said "rates". Same numbers, their words.
    {
      question: "Where can I see motor yacht charter Greece prices in one place?",
      answer:
        "This page is that place: the rate matrix above lists motor yacht charter Greece prices by yacht size and season, drawn from the live rate cards of the yachts listed here rather than from scraped listings. " +
        "Prices are quoted in EUR as base weekly rates; the all-in figure adds APA provisioning, VAT at the yacht's certified rate and a discretionary gratuity, and the breakdown table shows exactly how each layer lands.",
    },
    // 2026-08-01 SD-2 (2027 sweep): the rates page answered "how much"
    // only for 2026; buyers shopping now are pricing 2027.
    {
      question: "What will a weekly motor yacht charter cost in 2027?",
      answer:
        "Owners publish their 2027 rate cards through the autumn, so the bands on this page are the live reference until then. On a same-yacht basis Greek rates have been stable or modestly up year over year, and the 2027 calendars are already open: the practical move for a July or August 2027 week is to reserve the yacht now and let the rate confirm when the card publishes, which is exactly how the early bookings are being written.",
    },
    {
    ...
    },
    {
      ...
      answer:
        `Under the 2026 Greek regime, commercial crewed charters longer than 48 hours carry a reduced statutory ${VAT_PCT}% VAT rate, and certification-based reductions bring most invoiced rates to 5.2-12%; the exact figure for each yacht is stated in the written quote. Short charters under 48 hours, static stays, and bareboat charters without crew start from the standard 24%.`,
    },
    {
      question: "How much is a motor yacht charter for the week?",
      answer:
        `On a 30m motor yacht in peak season (about ${euro(e.total)} all-in for the week), that is the figure to compare. Smaller yachts and the shoulder or off-peak months bring the weekly figure down considerably.`,
    },
    {
