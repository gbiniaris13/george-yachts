...
export async function eligibleYachtForBridge() {
  ...
  // 2026-09-08 — the same gate the client newsletter's selector got today,
  // and this letter needs it more. The Bridge goes to travel advisors, so
  // it is the B2B channel George named first when he set the rule: the
  // twenty-seven hulls licensed for the website may not appear in it at
  // all. Cleared nobody when the policy cannot be read.
  candidates = await filterOutwardAllowed(candidates);
