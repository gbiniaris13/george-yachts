...
//   • Sign-off "- George" (short hyphen, no period; the em dash was
//     removed 2026-08-07, it reads as machine-written)
...
import { FORBIDDEN_COMPETITOR_NAMES } from "./validator";
import { awardsFor } from "@/lib/yachtAwards";

// ─── Sign-offs (mirrors validator.SIGN_OFFS) ───────────────────────
// 2026-09-03 — the leading ", George" was the ghost of an em dash
// removed on 7/8 (", George" is what "— George" became). Dash-free
// sign-offs that read as a letter, title per the house rule
// (Founder and Managing Broker, never owner).
const SIGNOFFS = {
  bridge: "Warm regards from Athens,\nGeorge\n\nGeorge P. Biniaris\nFounder and Managing Broker, George Yachts\ngeorgeyachts.com",
  wake: "George P. Biniaris\nFounder and Managing Broker, George Yachts\ngeorgeyachts.com",
...
// No trailing dashes: the house bans long dashes and the validator
// counted the old opener's "-" as one, blocking the issue.
const OPENERS = {
  bridge: "From Athens.",
...
// House rule, enforced at the final assembly step: no em or en dashes
// anywhere in a newsletter body. Sanity descriptions, blog excerpts
// and George's free text all arrive with them; between words they
// become a comma pause, between digits a plain hyphen.
...
function cleanClassFragment(subtitle) {
  ...
  for (const seg of segments) {
    ...
    // A shipyard whose name is also a brokerage (Camper & Nicholsons)
    // would trip the validator's §13.2 competitor rule and block the
    // whole letter. The hull keeps her page; the class line skips it.
    const low = seg.toLowerCase();
    if (FORBIDDEN_COMPETITOR_NAMES.some((n) => low.includes(n))) continue;
...
export function buildAnnouncement({
  ...
  return {
    subject,
    preheader: preheaderFrom(body, `New in this house, ${yacht.name}.`),
...
export function buildOffer({
  ...
  george_angle,     // window + region or human framing (§3.5: "Window + region")
  // posture kept for back-compat but no longer auto-emitted in body
  posture: _posture,
}) {
  const opener = OPENERS[stream] ?? OPENERS.bridge;
  ...
  const subject =
    stream === "bridge"
      ? `A note on availability${subjectName}`
      : stream === "wake"
        ? `Greek-waters availability${subjectName}`
        : `Availability note${subjectName}`;

  // Yacht context — only when George picked a specific boat.
  // "Greece" alone reads thin in a sentence; widen to "Greek waters".
  let yachtLine = "";
  if (yacht) {
    const cls = cleanClassFragment(yacht.subtitle);
    const rawRegion = yacht.cruisingRegion?.trim() || "";
    const region =
      !rawRegion || /^greece$/i.test(rawRegion) ? "Greek waters" : rawRegion;
    yachtLine = cls
      ? `On ${yacht.name}, a ${cls} cruising ${region}.`
      : `On ${yacht.name}, cruising ${region}.`;
  }

  // Lead — soft urgency, no marketing. George's angle (window + region
  // or framing) IS the lead. If absent, fallback is a posture sentence
  // that doesn't fabricate specifics.
  const lead = george_angle?.trim()
    ? george_angle.trim()
    : "A few of the right pieces are lining up this stretch.";

  // Per-stream close per §3.5. Bridge speaks directly to the client
  // (they ARE the someone); Wake speaks about THEIR clients.
  const close =
    stream === "wake"
      ? "Reply if any of your clients are still deciding on Greece this summer."
      : "If a Greek-waters week has been on your mind, hit reply. I'll pull what's actually open. No calendar, just a conversation.";

  const link = yacht?.page_url ? `\n\nMore on her: ${yacht.page_url}` : "";

  // Assemble with explicit blank lines between sections.
  const sections = [opener, lead];
  if (yachtLine) sections.push(yachtLine);
  sections.push(close);
  const body = sections.join("\n\n") + link;

  return {
    subject,
    preheader: preheaderFrom(body, "A note on availability."),
    body_text: withSignoff(stream, body),
    content_type: "offer",
    hero_image_url: yacht?.hero_image_url ?? null,
...
export function buildIntel({
  ...
  // Defensive: when George copies a signal from another doc, it often
  // ends with a trailing "Source: ..." attribution line. Strip it so
  // we don't double-emit when source_note is also provided.
  // If source_note is EMPTY but signal has a trailing Source: line,
  // promote the inline value into source_note so attribution isn't
  // lost. The separate field handles the parenthetical formatting.
  const inlineSourceMatch = String(signal_text).match(
    /\n+\s*Source\s*:\s*([^\n]+?)\s*$/i,
  );
  if (inlineSourceMatch) {
    signal_text = String(signal_text)
      .replace(/\n+\s*Source\s*:\s*[^\n]*$/i, "")
      .trim();
    if (!source_note || !String(source_note).trim()) {
      source_note = inlineSourceMatch[1].trim();
    }
  } else {
    signal_text = String(signal_text).trim();
  }
...
// ─── §8.6 — Spotlight (existing yacht in rotation) ─────────────────
//
// Used by the full-auto rotation engine to re-present a yacht already
// in the fleet. Same voice/whitespace discipline as the announcement,
// but WITHOUT the "a new boat just joined" line (that would be a false
// claim for an existing boat) and WITHOUT availability/offer framing
// (which would over-index on selling vs. the research's 80/20 rule).
// Storytelling from real Sanity fields only — never fabricates.

export function buildSpotlight({
  stream,
  yacht,            // from getYachtForNewsletter
  george_angle,     // optional per-issue positioning override
  link_label = "See her on the site",
}) {
  if (!yacht) throw new Error("yacht required for spotlight");

  const opener = OPENERS[stream] ?? OPENERS.bridge;

  const subject =
    stream === "bridge"
      ? `From our Greek fleet: ${yacht.name}`
      : stream === "wake"
        ? `For your file: ${yacht.name} (Greek waters)`
        : stream === "compass"
          ? `Fleet note, ${yacht.name}`
          : `Από τον στόλο μας: ${yacht.name}`;
  ...
  // Region line — widen bare "Greece" to "Greek waters" for prose.
  const rawRegion = yacht.cruisingRegion?.trim() || "";
  const region =
    !rawRegion || /^greece$/i.test(rawRegion) ? "Greek waters" : rawRegion;
  const cruiseLine = `\n\nShe cruises ${region}.`;

  // Stable, boat-level insider note (same source priority as announce).
  const stableInsider = buildInsiderBlockFromStable(yacht);
  let insiderBlock = "";
  if (stableInsider) {
    insiderBlock = `\n\nWhat I know about her:\n${stableInsider}`;
  ...
  // Positioning: George's angle wins, else Sanity one-liner, else omit.
  let positioningLine = "";
  const angleClean = george_angle?.trim();
  const posClean = yacht.positioning_one_liner?.trim();
  if (angleClean) {
  ...
  const cta =
    stream === "wake"
      ...
      : stream === "compass"
        ? ""
        : `\n\nIf she's the right fit for someone you know, hit reply.`;

  const pageLink = yacht.page_url ? `\n\n${link_label}: ${yacht.page_url}` : "";

  const head = [opener, "", classLine, specs]
  ...
  const body =
    head + cruiseLine + insiderBlock + positioningLine + cta + pageLink;

  return {
    subject,
    preheader: preheaderFrom(body, `${yacht.name}, Greek waters.`),
    body_text: withSignoff(stream, body),
    content_type: "spotlight",
    hero_image_url: yacht.hero_image_url ?? null,
    audience_suggested: routeAudience("spotlight").suggested,
  };
}

// ─── Dispatcher ─────────────────────────────────────────────────────

// ─── Fleet update (several new yachts in one letter) ───────────────
//
// 2026-09-03, George's call. The fleet grew by 23 hulls in three
// weeks and the one-yacht announcement at a monthly cadence would
// have taken two years to introduce them. One letter, grouped by
// type, one line of facts per yacht from Sanity, the documented
// awards from lib/yachtAwards.js where they exist, no prices.
//   bridge → story voice, wake → inventory voice.

const CATEGORY_LABEL = {
  "motor-yachts": "Motor yachts",
  "power-catamarans": "Power catamarans",
  "sailing-catamarans": "Sailing catamarans",
  "sailing-monohulls": "Sailing yachts",
};
...
function fleetUpdateLine(y, stream) {
  const cls = cleanClassFragment(y.subtitle);
  const specs = buildSpecLine(y);
  const head = [y.name, cls].filter(Boolean).join(", ");
  const parts = [`${head}${specs ? `. ${specs}` : ""}.`];
  // Wake gets the "who she suits" beat too; bridge keeps it lighter.
  const ideal = y.idealFor?.trim().replace(/[.!?,;:]+$/, "");
  if (stream === "wake" && ideal && !failsVoiceGate(ideal)) {
    parts.push(`Suits ${ideal.charAt(0).toLowerCase()}${ideal.slice(1)}.`);
  }
  const awards = awardsFor(y.slug).slice(0, 1);
  for (const a of awards) {
    const who = a.kind === "crew" ? " (crew award)" : "";
    parts.push(`${a.organiser} ${a.year}: ${a.award}${who}.`);
  }
  parts.push(`${y.page_url}`);
  return parts.join(" ");
}

export function buildFleetUpdate({ stream, yachts, george_angle }) {
  if (!Array.isArray(yachts) || yachts.length === 0) {
    throw new Error("yachts required for fleet_update");
  }
  const opener = OPENERS[stream] ?? OPENERS.bridge;
  const n = yachts.length;
  const groups = new Map();
  for (const y of yachts) {
    const key = y.category ?? "motor-yachts";
  ...
  const counts = orderedKeys
    .map((k) => `${groups.get(k).length} ${CATEGORY_LABEL[k]?.toLowerCase() ?? "yachts"}`)
    .join(", ");

  const lead =
    stream === "wake"
      ? `${n} yachts joined the Greek-waters fleet for 2027: ${counts}. Specs below, every page linked, rates and availability by reply.`
      : `The fleet has grown for 2027: ${counts}, each with her own page and photographs on the site. A short introduction to each, in the order I would show them to you.`;

  const sections = orderedKeys.map((k) => {
    const label = CATEGORY_LABEL[k] ?? "Yachts";
    const lines = groups.get(k).map((y) => fleetUpdateLine(y, stream));
    return `${label}\n${lines.join("\n\n")}`;
  });
  ...
  const cta =
    stream === "wake"
      ? "If any of them fits a brief you are working on, reply with the dates and party size."
      : "If one of them catches your eye for next summer, reply and I will tell you what her weeks look like.";

  const body = `${opener}\n\n${lead}${angle}\n\n${sections.join("\n\n")}\n\n${cta}`;
  const subject =
    stream === "wake"
      ? `For your file: ${n} new yachts in Greek waters for 2027`
      : `New for 2027: ${counts}`;
  ...
  return {
    ...
    hero_image_url: yachts[0]?.hero_image_url ?? null,
    audience_suggested: routeAudience("fleet_update").suggested,
  };
}
...
export function assembleBody(args) {
  switch (args.content_type) {
    case "fleet_update":
      return buildFleetUpdate(args);
    case "announcement":
      return buildAnnouncement(args);
    case "spotlight":
      return buildSpotlight(args);
    case "offer":
