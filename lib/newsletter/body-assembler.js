...
/**
 * Update 2 §5.3 — Build the "What I know about her:" block from
 * structured Sanity fields rather than the free-text georgeInsiderTip.
 * Each line is constructed deterministically so it never violates §13
 * voice rules (no em-dashes, no banned phrases, no ALL CAPS).
 *
 * Order matters — lead with the captain (the human face of the boat),
 * then crew, then seasons, then credentials (gated).
 *
 * Returns an empty string if no structured field is populated. The
 * announce builder falls back to the legacy georgeInsiderTip (gated)
 * in that case, then to omission per §3.2.1.
 *
 * @param {object} y       yacht record
 * @param {object} opts
 *   include_captain_credentials  bool — Composer UI checkbox
 */
function buildInsiderBlockFromStructured(y, opts = {}) {
  const lines = [];

  // Captain line — most powerful when both name + tenure are present.
  // Credentials, if opted-in, are emitted as a parenthetical so the
  // sentence stays natural prose rather than a bulleted-feeling pair.
  const credSegment =
    opts.include_captain_credentials &&
    y.captain_credentials_short &&
    !failsVoiceGate(y.captain_credentials_short)
      ? ` (${y.captain_credentials_short})`
      : "";
  if (y.captain_name && y.captain_on_board_since) {
    lines.push(
      `Captain ${y.captain_name}${credSegment} has been on board since ${y.captain_on_board_since}.`,
    );
  } else if (y.captain_name) {
    lines.push(`Captain ${y.captain_name}${credSegment} runs this boat.`);
  }

  // Chef / Cook line. chef_specialty_one_line is voice-gated because
  // it's free text and could contain banned phrases. Trailing period
  // stripped to avoid double-period when embedded in our sentence.
  const chefSpec = y.chef_specialty_one_line?.trim().replace(/[.!?]+$/, "");
  if (y.chef_name && chefSpec && !failsVoiceGate(chefSpec)) {
    lines.push(`${y.chef_name}'s cooking is ${chefSpec}.`);
  } else if (y.chef_name) {
    lines.push(`${y.chef_name} runs the galley.`);
  } else if (chefSpec && !failsVoiceGate(chefSpec)) {
    lines.push(`The cooking is ${chefSpec}.`);
  }

  // Seasons-active line — only mention when meaningful (≥2 seasons).
...
export function buildAnnouncement({
  ...
  // "What I know about her:" block. Update 2 §5.3 priority order:
  //   1. Structured fields from Sanity (captain_name, chef_name, etc.)
  //      — produces deterministic, voice-clean sentences
  //   2. Fallback to legacy georgeInsiderTip if structured is empty
  //      AND the tip passes the voice gate
  //   3. Otherwise OMIT entirely (§3.2.1: never fabricate)
  const structuredInsider = buildInsiderBlockFromStructured(yacht, {
    include_captain_credentials,
  });
  let insiderBlock = "";
  if (structuredInsider) {
