...
// Site brand mark — vector yacht symbol on Deep Navy rounded square,
// matching the live nav drawer (NavDrawerSystem.jsx). Hosted under
// /public/images/ so the URL is stable across deploys. Modern email
// clients (Gmail web/mobile, Apple Mail, Yahoo, Outlook web/mobile)
// render SVG via <img>. Outlook desktop falls back to the alt text
// + the wordmark below — brand identity is still readable.
const BRAND_MARK_URL =
  "https://georgeyachts.com/images/newsletter-brand-mark.svg";
  ...
  return `
    <tr>
      <td style="padding:36px 32px 0 32px;text-align:center;">
        <a href="https://georgeyachts.com" style="text-decoration:none;border:0;outline:none;display:inline-block;line-height:0;">
          <img src="${BRAND_MARK_URL}" width="44" height="44" alt="George Yachts" style="display:block;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic;width:44px;height:44px;margin:0 auto;" />
        </a>
        <div style="font-family:'Cormorant Garamond',Georgia,'Times New Roman',serif;font-weight:300;letter-spacing:0.22em;font-size:18px;line-height:1;color:${COLORS.ink};margin-top:14px;">
          GEORGE&nbsp;YACHTS
        </div>
        <div style="font-family:-apple-system,BlinkMacSystemFont,'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:8px;letter-spacing:0.32em;color:${COLORS.gold};text-transform:uppercase;margin-top:6px;">
          Brokerage&nbsp;House&nbsp;LLC
        </div>
      </td>
    </tr>
    <tr>
      <td style="padding:22px 32px 8px 32px;text-align:center;">
        <div style="height:1px;background:${COLORS.gold};width:60px;margin:0 auto 16px auto;line-height:1px;font-size:0;">&nbsp;</div>
        <div style="font-family:'Cormorant Garamond',Georgia,'Times New Roman',serif;font-weight:300;letter-spacing:0.18em;font-size:26px;line-height:1;color:${COLORS.ink};">
          ${escapeHtml(label)}
        </div>
      </td>
    </tr>`;
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:${COLORS.paper};">
  ...
  // Plain-text counterpart: brand → stream label → body → footer.
  const text = [
    `GEORGE YACHTS · BROKERAGE HOUSE LLC`,
    `${STREAM_LABELS[stream] ?? "THE BRIDGE"}`,
    `${"-".repeat(40)}`,
    "",
