...
import { outwardBlockReason, filterOutwardAllowed } from "@/lib/socialPolicy";
...
//
// 2026-04-29 Update 2 amendment: crew / captain / chef IDENTITIES are
// volatile (a captain can change between bookings). The /announce body
// MUST NOT reference named crew. captain_name / chef_name /
// captain_credentials_short remain in the projection as record-of-truth
// for admin contexts (e.g. /intel safety briefings) but the
// announce body-assembler ignores them. Stable yacht info only:
// description, idealFor, positioning_one_liner, seasons_active_count.
//
// description is fetched as plain text via pt::text() since it's
// stored as Portable Text (block array) in Sanity — the body
// assembler needs a string it can truncate to first 1-2 sentences.
const YACHT_PROJECTION = `{
  _id,
  "slug": slug.current,
  name,
  subtitle,
  "description_text": pt::text(description),
  description,
  georgeInsiderTip,
  length,
  yearBuiltRefit,
...
export async function getYachtForNewsletter(slug) {
  ...
  // 2026-09-08: yachts listed under a website-only permission never
  // travel in a newsletter, client or advisor.
  const blocked = await outwardBlockReason(slug, "newsletter");
  ...
  try {
    ...
    return {
      ...
      yacht: {
        ...
        description_text: y.description_text ?? null,
...
/**
 * Fetch several yachts by slug for a fleet-update letter (2026-09-03).
 * Same no-price projection discipline as the single fetch: nothing
 * bookable leaves this module. Order follows the slug list given.
 */
export async function getYachtsForFleetUpdate(slugs) {
  const wanted = Array.isArray(slugs)
    ? slugs.map((s) => String(s || "").trim()).filter(Boolean)
    : [];
  if (wanted.length === 0) return { ok: false, error: "slugs required", yachts: [] };
  const cleared = await filterOutwardAllowed(
    wanted.map((slug) => ({ slug })),
  );
  const clearedSlugs = cleared.map((c) => c.slug);
  if (clearedSlugs.length === 0) {
    return { ok: false, error: "no requested yacht is cleared for a newsletter", yachts: [] };
  }
  try {
    const rows = await sanityClient.fetch(
      `*[_type == "yacht" && slug.current in $slugs]{
        _id,
        "slug": slug.current,
        name,
        subtitle,
        ...
        cruisingRegion,
        idealFor,
        "images": images[0..1]{ "url": asset->url, "alt": alt }
      }`,
      { slugs: clearedSlugs },
    );
    const bySlug = new Map((rows ?? []).map((y) => [y.slug, y]));
    const yachts = wanted
      .map((s) => bySlug.get(s))
      .filter(Boolean)
      .map((y) => ({
    ...
    return { ok: yachts.length > 0, yachts, error: yachts.length ? null : "no yachts found" };
  } catch (err) {
...
export async function listYachtsForComposer() {
  try {
    ...
    // Website-only yachts never appear in the composer, so they cannot be
    // picked for a newsletter by hand either.
    const cleared = await filterOutwardAllowed(rows ?? []);
    return { ok: true, yachts: cleared };
  } catch (err) {
    return { ok: false, error: `sanity fetch failed: ${err?.message || err}`, yachts: [] };
  }
}
...
/**
 * Parse a blog article URL into a Sanity slug. Accepts:
 *   - https://georgeyachts.com/blog/<slug>
 *   - https://georgeyachts.com/blog/<slug>?utm_=…
 *   - https://georgeyachts.com/blog/<slug>/
 *   - /blog/<slug>
 *   - just the bare slug ("hushpitality-greek-yacht-charter-quiet-luxury-2026")
 *
 * Returns null if the URL doesn't match the blog pattern (so the
 * caller can refuse to compose rather than silently picking a
 * wrong post).
 */
export function slugFromBlogUrl(input) {
  if (!input || typeof input !== "string") return null;
  const cleaned = input.trim();
  if (!cleaned) return null;
  // Bare slug: kebab-case, no protocol, no slashes
  if (/^[a-z0-9][a-z0-9-]*$/i.test(cleaned)) {
    return cleaned.toLowerCase();
  }
  try {
    // Use a base so relative paths parse too. Then enforce host.
    const u = new URL(cleaned, "https://georgeyachts.com");
    if (
      u.hostname !== "georgeyachts.com" &&
      u.hostname !== "www.georgeyachts.com" &&
