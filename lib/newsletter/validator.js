...
//
// Split into two tiers (2026-06-03):
//
//   • COMPETITOR brand names — naming a rival house in ANY client-
//     facing piece is reputational damage, so these always hard-block,
//     regardless of content type.
//
//   • GENERIC industry terms + trade-body / standard-contract names
//     ("central agent", "MYBA", etc.). In a client LETTER these reveal
//     the agency chain and must block. But a BLOG recap points to
//     George's own published, editorially-reviewed educational posts
//     (e.g. "Greek Yacht Charter Contract Explained: MYBA Standard
//     Terms"), where naming the MYBA standard contract or explaining
//     the central-agent role IS the subject. For content_type "blog"
//     these downgrade to a warning so the recap can ship.
// Exported so the body assembler can keep a builder name that doubles
// as a brokerage name (Camper & Nicholsons built NORTHWIND II in 1966)
// out of the class line before the validator ever sees it.
export const FORBIDDEN_COMPETITOR_NAMES = [
  "iyc",
...
// Generic industry terms + trade-body / standard-contract names.
// Block in client letters; warn-only for blog recaps (see note above).
const INDUSTRY_GENERIC_TERMS = [
  "myba",  // trade body + standard charter-agreement name
  "iyba",  // trade body
  "central agent",
  "central agency",
  "fleet manager",
...
// 2026-09-03 — the old pattern was `\s+(WORD)\b` with the /i flag,
// which fired on the ordinary lowercase word "new" in "A new boat
// just joined" and blocked EVERY announcement since 13/8 (eight
// consecutive held-back issues, nothing sent for six weeks). Badge
// syntax is a separator followed by an UPPERCASE badge word; match
// exactly that, case-sensitively.
const BADGE_PATTERNS = [
  /[★⭐]/,                                                            // any star char
  new RegExp(`(?:^|[—–|•·]\\s*)(?:${BADGE_WORDS.join("|")})\\b`, "m"), // "— EXCLUSIVE", "| NEW"
];
...
// Counts long dashes only. The previous version counted every "-",
// so "Greek-waters file -" (one hyphenated word plus an opener dash)
// read as two em dashes and blocked the issue. Hyphens inside words
// are not the rule's target; em and en dashes are.
...
// initial, "George P. Biniaris" everywhere it is used as a formal
// signature. Bridge stays the warm "George".
//
// 2026-08-07: the em dash left the sign-offs. George's standing rule is
// that a long dash reads as machine-written, and the sign-off is the last
// thing a reader sees. The old em-dash forms stay in the accepted list so
// an issue drafted before today still validates instead of failing at send.
// Mirrors body-assembler SIGNOFFS (2026-09-03: dash-free sign-offs,
// the old ", George" was a stripped em dash left behind as a comma).
const SIGN_OFFS = {
  ...
  compass: ["George P. Biniaris\nGeorge Yachts", "- George P. Biniaris", "— George P. Biniaris"],
  greece: ["Γιώργος Π. Μπινιάρης", "- Γιώργος", "— Γιώργος"],
};
...
export function validateNewsletterContent(args) {
  ...
  // Competitor brand names always hard-block (naming a rival house in
  // any client-facing piece is reputational damage). Generic industry
  // terms / trade-body names block for client letters but downgrade to
  // a warning for blog recaps, which point to George's own published
  // educational posts where naming e.g. the MYBA contract is the point.
  const competitorHits = FORBIDDEN_COMPETITOR_NAMES.filter((name) =>
    haystack.includes(name),
  );
  if (competitorHits.length > 0) {
    violations.push({
      rule: "§13.2, no central agent or management company names",
      evidence: competitorHits,
      severity: "block",
    });
  }
  const industryTermHits = INDUSTRY_GENERIC_TERMS.filter((name) =>
    haystack.includes(name),
  );
  if (industryTermHits.length > 0) {
    const isBlog = contentType === "blog";
    violations.push({
      rule: isBlog
        ? "§13.2, industry/trade-body term in blog recap (review only, recap links to published post)"
        : "§13.2, no central agent or management company names",
      evidence: industryTermHits,
      severity: isBlog ? "warn" : "block",
    });
  }
