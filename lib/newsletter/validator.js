...
// Badge syntax: stars + ALL-CAPS-word-after-em-dash patterns
const BADGE_PATTERNS = [
  /[★⭐]/,                           // any star char
  /—\s+[A-Z]{3,}/,                  // "— BEST" "— EXCLUSIVE" etc.
];

// Em-dash counter helpers — Update 2 §3.1.4: max 1 per sentence,
// max 2 per body paragraph. We split on sentence terminators and
// blank lines respectively.
function countEmDashesInSentence(s) {
...
function tooManyEmDashes(body) {
  ...
  for (const s of sentences) {
    if (countEmDashesInSentence(s) > 1) {
      return { rule: "sentence", evidence: s.slice(0, 120) };
    }
  }
  // Per-paragraph check
  ...
  for (const p of paragraphs) {
    if (countEmDashesInSentence(p) > 2) {
      return { rule: "paragraph", evidence: p.slice(0, 200) };
    }
  }
  return null;
}
...
export function validateNewsletterContent(args) {
  ...
  if (dashIssue) {
    violations.push({
      rule: `Update 2 §3.1.4 — em-dash overuse (per-${dashIssue.rule} limit exceeded)`,
