...
//
// FROM address policy:
//   Default `newsletter@send.georgeyachts.com` (Resend's recommended
//   subdomain pattern). DKIM is on the apex (resend._domainkey) so
//   DMARC aligns via DKIM. SPF check happens on the send subdomain
//   which carries Resend's SPF, so SPF aligns too. Reply-to stays on
//   the apex `george@georgeyachts.com` so when subscribers reply, it
//   lands in George's normal Gmail inbox.
//
...
const DEFAULT_FROM =
  ...
  "George Yachts <newsletter@send.georgeyachts.com>";
