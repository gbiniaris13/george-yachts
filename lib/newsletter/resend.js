...
import { canSendNow, recordSend } from "./quota";
import { markEvent } from "./engagement";
import { recordIssueEvent } from "./issue-stats";
import { maybeFireQuotaAlert } from "./quota-alerts";
...
export async function sendNewsletterEmail({
  ...
  // Per-day Resend quota gate. Soft-cap at 95/day (Resend's hard cap is
  // 100/day on free). When this trips, callers (approve, topup, welcome
  // flow) should queue the remaining recipients into the draft's
  // pending set so the daily-flush cron picks them up tomorrow.
  const quota = await canSendNow();
  if (!quota.ok) {
    return {
      ok: false,
      error: `rate_limited (${quota.reason}: daily ${quota.daily_used}/95, monthly ${quota.monthly_used}/2900)`,
      rate_limited: true,
      reason: quota.reason,
  ...
  if (!res.ok) {
    ...
    // 429 (rate limit) and 402 (payment required / quota exhausted) get
    // routed through the same queue path as our internal soft-cap. This
    // catches the case where our KV counter is out of sync with Resend's
    // own enforcement (e.g. a manual unrecorded send pushed Resend over
    // first).
    if (res.status === 429 || res.status === 402) {
      return {
        ok: false,
        error: `rate_limited (resend HTTP ${res.status}): ${detail.slice(0, 200)}`,
        rate_limited: true,
        status: res.status,
      };
    }
  ...
  // Successful send — record against the daily + monthly quota.
  await recordSend();
  // Phase 5.1 — baseline engagement record (per-recipient).
  markEvent({ email: recipient, event: "send" }).catch(() => {});
  // Phase 6.1 — baseline per-issue counter. We read the tags we just
  // set on the outgoing email to extract stream + issue. This means
  // sent counts work even if the Resend webhook isn't wired yet;
  // delivered/opened/clicked still need the webhook.
  let tagStream = null;
  let tagIssue = null;
  for (const t of tags || []) {
    if (t?.name === "stream") tagStream = t.value;
    else if (t?.name === "issue") tagIssue = t.value;
  }
  if (tagStream && tagIssue) {
    recordIssueEvent({
      stream: tagStream,
      issue: tagIssue,
      event: "sent",
  ...
  // Phase 6.3 — fire 80%/90%/95% monthly-cap alert if we just crossed
  // a threshold for the first time this month. Once-per-month flag
  // ensures no double-noise.
  maybeFireQuotaAlert().catch(() => {});
