...
export const serviceSchema = {
  ...
  "description":
    "Personally-vetted crewed yacht charters across the Greek islands - Cyclades, Ionian, Saronic. 63 yachts curated from a working broker in Athens. MYBA-standard contracts, full APA + VAT transparency, reply within 24 hours.",
  "provider": {
    "@type": "Organization",
...
export async function getServiceSchemaWithReviews() {
  ...
  const agg = await fetchReviewAggregate();
  const ratingBlock = aggregateRatingForSchema(agg);
  if (!ratingBlock) return serviceSchema;
  // 2026-06-28 (GEO research Tier 2) — when the >=3 gate passes, attach
  // BOTH the AggregateRating (the number) and the individual Review nodes
  // (the quotable client words AI engines lift into comparison answers).
  const reviews = await fetchReviewsForSchema();
  // 2026-06-28 (GEO research Tier 3) — rolling priceValidUntil on the
  // umbrella offers so AI/Google never see a stale price. Mirrors the
  // per-yacht page logic: season end (Oct 31), rolled to next year once
  // passed. This function runs server-side under ISR, so it stays fresh.
  const _now = new Date();
  ...
  const out = { ...serviceSchema, offers: offersWithValidity, aggregateRating: ratingBlock };
