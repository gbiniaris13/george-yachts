...
const COHORTS = {
  ...
  "motor-yacht": [
    ...
    // 2026-09-07: /weekly-motor-yacht-charter-greece and
    // /motor-yacht-charter-rhodes were listed here AND in "weekly-rates" and
    // "dodecanese"; a page belongs to exactly one cohort (last declaration
    // wins), so both silently belonged elsewhere. Removed here, which also
    // brings this cohort to seven members so every member reaches every
    // sibling in six slots; the speed page had zero inbound links before.
    ...
    // 2026-09-07: /motor-yacht-charter-saronic-gulf moved to the "saronic"
    // cohort below. With ten members and six slots it was drawing zero
    // inbound links from this family; in a cohort of seven it reaches all.
  ],
  ...
  "honeymoon": [
    ...
    // 2026-09-07 (plan item 12): the Santorini couples page had zero
    // inbound links from the engine; it is the same reader.
    "/yacht-charter-santorini-couples",
  ],
  ...
  // 2026-09-04 (George: American guests, weekly, fully crewed). Search
  // Console, United States, 90 days: the all-inclusive page sits at 1.8 on
  // 911 impressions and belonged to no cohort at all, so nothing on the
  // site surfaced it as a sibling; the American-clients page had one
  // footer link and no siblings. This cohort binds the pages an American
  // reads before sending the brief: the fixed-price week, the US entity
  // and contract, the private and luxury tiers, the crewed-islands
  // overview and the credentials with the registers by name.
  "american-crewed": [
    ...
    // 2026-09-07 (plan item 12): the UK clients page, same contract, same
    // fixed-price week, zero inbound links from the engine before this.
    "/yacht-charter-greece-uk-clients",
  ],
  // 2026-09-06 (plan item 12, the Cyclades). /destinations/cyclades carries
  // the region's head terms, "cyclades yacht charter" and "yacht charter
  // cyclades", 250 impressions a month at position 31, and it was not in
  // this catalog at all: its only inbound links were the footer, the nav
  // drawer and the anchorage guides. The page itself is deep (route,
  // wind, anchorages, eight answers); what it lacked was a family. These
  // are the pages a reader planning a Cyclades week moves between. The
  // island pages stay out on purpose: a cohort of eight would push their
  // own motor-yacht and itinerary siblings off the Mykonos and Santorini
  // blocks, and those are money pages.
  cyclades: [
    "/destinations/cyclades",
    "/7-day-cyclades-yacht-charter-itinerary-2026",
    "/athens-to-mykonos-by-motor-yacht",
    "/athens-vs-mykonos-vs-lefkada-yacht-charter-departure",
    "/blog/the-7-day-cyclades-itinerary-what-your-captain-won-t-tell-you-until-you-re-onboard",
  ],
  // 2026-09-06 — the Ionian, same reasoning. The hub draws 13 impressions
  // a month and the region's itinerary, two-week, sailing and comparison
  // pages belonged to no cohort, so none of the five could reach another.
  ionian: [
    "/destinations/ionian",
    "/7-day-ionian-yacht-charter-itinerary-2026",
    "/yacht-charter-ionian-2-weeks",
    "/sailing-yacht-charter-ionian",
    "/cyclades-vs-ionian-yacht-charter",
  ],
  // 2026-09-07 (plan item 12, after the depth pass of 6 and 7 September).
  // An inbound audit of the 44 pages that received depth found 35 of them
  // with ZERO links from this engine: every bestYachts page, most
  // bottom-funnel pages and every anchorage guide. A page nobody links to is
  // a page Google treats as nobody's. Six cohorts of at most seven members
  // each, so that with six slots every member reaches every sibling.
  //
  // saronic: the Athens home water. The hub, the motor-yacht page, the
  // short-notice page (whose programme is the five-night Saronic week) and
  // the four island guides the week is built from.
  saronic: [
    "/destinations/saronic",
    "/motor-yacht-charter-saronic-gulf",
    "/last-minute-yacht-charter-greece-2026",
    "/yacht-charter-hydra-anchorages",
    "/yacht-charter-poros-anchorages",
    "/yacht-charter-spetses-anchorages",
    "/yacht-charter-dokos-anchorages",
  ],
  // family: the two family pages, the Ionian guides the family week is
  // built from, and the Kefalonia catamaran page that argues the same case.
  family: [
    "/yacht-charter-greece-family-with-children",
    "/best-yachts-greece-families-children",
    "/yacht-charter-corfu-anchorages",
    "/yacht-charter-paxos-anchorages",
    "/yacht-charter-antipaxos-anchorages",
    "/catamaran-charter-kefalonia",
  ],
  // superyacht: the pillar, the two August pages (2,507 and 1,788
  // impressions in 28 days), the twelve-guest and stabilised lists, mega.
  superyacht: [
    "/superyacht-charter-greece",
    "/best-superyachts-greece-august",
    "/superyacht-charter-greece-august",
    "/best-yachts-greece-large-groups",
    "/best-yachts-greece-stabilizers-smooth-sailing",
    "/mega-yacht-charter-greece",
  ],
  // mykonos-week: the pages a party planning a Mykonos week moves between,
  // with the guides of the islands that week actually anchors at.
  "mykonos-week": [
    "/yacht-charter-mykonos-8-guests",
    "/yacht-charter-athens-to-mykonos",
    "/catamaran-charter-mykonos",
    "/yacht-charter-paros-anchorages",
    "/yacht-charter-antiparos-anchorages",
    "/yacht-charter-syros-anchorages",
    "/yacht-charter-kea-anchorages",
  ],
  // sailing-ionian: the sailing pillar, the named sailing list, and the
  // southern Ionian guides where the sailing week goes.
  "sailing-ionian": [
    "/sailing-yacht-charter-greece",
    "/best-sailing-yachts-greece",
    "/yacht-charter-lefkada-anchorages",
    "/yacht-charter-meganisi-anchorages",
    "/yacht-charter-ithaca-anchorages",
    "/yacht-charter-kalamos-anchorages",
    "/yacht-charter-zakynthos-anchorages",
  ],
  // cyclades-anchorages: the southern and western Cyclades guides, which
  // otherwise reach each other only through the region hub.
  "cyclades-anchorages": [
    "/yacht-charter-milos-anchorages",
    "/yacht-charter-amorgos-anchorages",
    "/yacht-charter-koufonisia-anchorages",
    "/yacht-charter-sifnos-anchorages",
    "/yacht-charter-folegandros-anchorages",
    "/yacht-charter-ios-anchorages",
    "/yacht-charter-naxos-anchorages",
  ],
  "journal-hubs": [
    "/blog",
...
function inferTags(urlPath) {
  ...
  // 2026-09-06 — the three region hubs (/destinations/cyclades, ionian,
  // saronic) carry no island slug, so nothing below could place them in
  // a region. The path is the region.
  if (slug.startsWith("destinations/")) tags.region = slug.slice("destinations/".length);
...
function buildCatalog() {
  ...
  // 2026-09-17: a long-tail page whose canonical is another page is not
  // offered as a related link either (today: the 2026 private page).
  for (const p of LONG_TAIL_PAGES) {
    if (p.canonical && p.canonical !== `https://georgeyachts.com${p.urlPath}`) continue;
    entries.push({ ...p, ...inferTags(p.urlPath), source: "long-tail" });
  }
  ...
  // 2026-09-14: an article whose canonical is another page is not offered as
  // a related link; its canonical twin is already in the catalog.
  for (const p of ARTICLES) {
    if (articleLinkPath(p) !== p.urlPath) continue;
    entries.push({ ...p, ...inferTags(p.urlPath), source: "article" });
  }
  ...
  // 2026-09-06 (plan item 12) — the three region hubs. Never in the catalog,
  // so no page on the site could surface them as a sibling; the Cyclades
  // hub, which holds the region's head terms, had the footer and the nav
  // as its only inbound links. Region tag from the path (inferTags), so a
  // Mykonos or Lefkada page sees its hub at region affinity; the Cyclades
  // hub also sits in its own cohort above.
  for (const d of Object.values(DESTINATIONS)) {
    entries.push({
      urlPath: `/destinations/${d.slug}`,
      h1: `${d.label} Yacht Charter`,
      eyebrow: "Region",
      source: "destination",
      category: "linkable-asset",
      region: d.slug,
...
// 2026-09-06 — every catalog URL, for the pre-push audit that counts how
// many related blocks a catalog change will re-stamp (lastmod is derived
// from visible text, so a link that moves on 60 pages is 60 fresh dates).
export function catalogPaths() {
  return CATALOG.map((e) => e.urlPath);
}
