...
import { humanDashesDeep } from "./humanDashes";
import { excludeRetiredYachts } from "./retiredYachts";
import { rewriteFleetTier, excludeMonohullsFromLists } from "./fleetTiers";
...
export function withRetry(client) {
  ...
  client.fetch = async (...args) => {
    // 2026-08-21 (section 6) — retired yachts never leave this building.
    // Forty-three files query `_type == "yacht"`; rewriting the GROQ here is
    // the only version of this that cannot be forgotten by the next one.
    // See lib/retiredYachts.js.
    // 2026-08-21 (section 9) — the two fleets are cut by sail against power
    // now, and the tier lives in the yacht's name rather than in a Sanity
    // field nobody can write to from here. Same chokepoint as the retired
    // set, for the same reason. See lib/fleetTiers.js.
    if (typeof args[0] === "string") {
      args = [excludeMonohullsFromLists(rewriteFleetTier(excludeRetiredYachts(args[0]))), ...args.slice(1)];
    }
    let lastErr;
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        // 2026-08-07 — every string that comes out of Sanity passes the
        // long-dash rule on the way in. Yacht subtitles, insider tips and
        // image alts are written in the Studio, not in this repo, so a
        // source sweep alone would leave them and would leave everything
        // George writes from now on. See lib/humanDashes.js.
        return humanDashesDeep(await originalFetch(...args));
      } catch (err) {
        lastErr = err;
