...
export const JOURNAL_CLUSTERS = [
  ...
  },
  {
    ...
    articles: [
      ...
      // last-cabin-standing removed 2026-07-02: the slug is retired
      // (see lib/retiredSlugs.js), so listing it here rendered a card
      // that 307'd back to /blog on every click.
    ],
    yachtFilter: 'fleetTier == "private"',
    cta: {
...
// 2026-07-30 — the reverse lookup. The cluster pages listed their articles
// but no article pointed back, so every hub sat on a single inbound link
// from /blog and readers who finished a post had no way into the series it
// belongs to.
//
// Returns ALL matching clusters, not the first. Seven posts sit in two to
// four clusters each (the hour-by-hour piece is in four), and a `find` would
// have handed every one of them to whichever cluster happens to be declared
// earliest in the array. /journal/choosing-a-yacht owns three articles and
// all three are claimed earlier, so it would have received nothing at all.
export function getClustersForPost(postSlug) {
  return JOURNAL_CLUSTERS.filter((c) => (c.articles || []).includes(postSlug));
}
