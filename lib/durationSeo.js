...
function buildPage(destinationSlug, durationKey) {
  ...
  return {
    ...
    faq: [
      { q: `How much does a ${tpl.durationName.toLowerCase()} yacht charter from ${dest.name} cost?`, a: `Base rates scale by yacht size. For ${tpl.durationName.toLowerCase()} the typical range starts around €${tpl.durationMin <= 3 ? "10" : tpl.durationMin <= 7 ? "25" : tpl.durationMin <= 10 ? "40" : "55"}K (smaller yachts) and runs to €${tpl.durationMin <= 3 ? "50" : tpl.durationMin <= 7 ? "140" : tpl.durationMin <= 10 ? "200" : "300"}K+ (30+ metre yachts), plus APA (25-30%) and Greek VAT at the certified rate (5.2-12% typical).` },
