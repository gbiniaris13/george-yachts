...
function buildPage(destinationSlug, durationKey) {
  ...
  const band = BASE_BAND[tpl.durationMin];
  ...
  return {
    ...
    seoDescription: `${tpl.durationName} crewed yacht charter from ${place}: base rates €${band.low}K to €${band.high}K plus APA and VAT, departure ${dest.departurePort}, day-by-day itinerary.`,
    ...
    insiderTips: [
      ...
      `Crew gratuity convention 10-15% of base rate paid in cash at end. Same convention regardless of charter duration.`,
    ...
    faq: [
      { q: `How much does a ${tpl.durationName.toLowerCase()} yacht charter from ${dest.name} cost?`, a: `Base rates scale by yacht size. For ${tpl.durationName.toLowerCase()} the typical range starts around €${band.low}K (smaller yachts) and runs to €${band.high}K+ (30+ metre yachts), plus APA (25-30%) and Greek VAT at the certified rate (5.2-12% typical). Crew gratuity is a further 10 to 15% of the base rate under MYBA convention.` },
