...
export const LAST_REFRESH = {
  ...
  // 2026-08-07 (second pass): George corrected a claim that was mine, not his.
  // The site said "Saturday to Saturday" in 25 places. He has no such limit:
  // the week can start on any day. Every claim about us was rewritten, the
  // market convention was kept where it is genuinely the market's, and not
  // being bound by it is now stated as the advantage it is.
  // 2026-08-08: the long dash left the site. 1,018 lines of source, five
  // locale files and every string Sanity returns. It touches nearly every
  // page, so every family advances.
  // 2026-08-08 (τρίτο πέρασμα): title-length rule so the brand suffix stops
  // truncating deliberate titles, and /cost-calculator canonicalised to the
  // calculator that actually earns.
  // 2026-08-15, the Q4 Charter Index refresh. The Index was rebuilt from the
  // live fleet (type by size, no estimated regional cells), and every page
  // that quotes it by name had to move with it or start contradicting it:
  // /crewed-yacht-charter-greece, /charter-yacht-greece, /yacht-charter-greece-2027,
  // /weekly-yacht-charter-rates-greece and the nine Greek /el/* pages all
  // carried the old 15,000-40,000 catamaran ladder in their own words.
  // 2026-08-21 — the fleet was re-cut and the awards became a section, so
  // three families move and the rest deliberately do not.
  //
  // What genuinely changed as CONTENT:
  //   STATIC  the homepage gained the awarded band above the fleet split,
  //           with a placing, a competition, an organiser, a year and a rate
  //           on every card; both fleet cards were renamed and given
  //           subtitles, and the nav label changed.
  //   FLEET   the fleet is re-cut into Private (motor and power cats) and
  //           Sailing (sailing cats and monohulls) from the name prefix, the
  //           EUR 29,000 ceiling came off /explorer-fleet and revealed
  //           nineteen yachts that were being hidden, and seven withdrawn
  //           hulls left the listings behind 301s.
  //   HUBS    /award-winning-yacht-charter-greece now names sixteen yachts
  //           and thirty-two placings instead of a handful.
  //
  // What did NOT change, and therefore does not move: every island, article,
  // glossary, comparison and long-tail page. Their words are identical. The
  // gold went from 65% to 86% saturation across the whole site, which is a
  // real change to how it LOOKS and no change at all to what it SAYS.
  // Stamping 482 pages as freshly written because a hex changed is exactly
  // the noise the note at the top of this file warns against, and Google
  // learns to discount a site that cries wolf.
  STATIC: "2026-08-21",
  ...
  // 2026-08-15: the catamaran cost section and the Cyclades pricing FAQ were
  // quoting the Index's old regional bands, which no longer exist.
  YACHT_TYPES: "2026-08-15",
  ...
  // 2026-08-15: the family-yacht answer quoted the Index's old catamaran band.
  USE_CASES: "2026-08-15",
  LONG_TAIL: "2026-08-08",
  // 2026-08-06: Mykonos vs Santorini retitled for the route intent and made
  // the single canonical of a duplicate pair.
  COMPARISONS: "2026-08-08",
  ...
  // 2026-08-08: the APA truth pass. The pillar claimed a "standard rate" of
  // 30-35% that MYBA does not set, and the pricing guide's hidden-costs list
  // was written for someone chartering a 50 metre with a helipad rather than
  // for the person actually asking the question.
  // 2026-08-15: the Athens cost FAQ quoted the Index's old catamaran band.
  ARTICLES: "2026-08-15",
  ...
  DEST_COMPARISONS: "2026-08-08",
  // 2026-08-06 — bumped with the sitewide footer rollout. These pages did
  // not change in wording, they changed in shape: each one went from roughly
  // 40 internal links to roughly 145, plus the legal links they never had.
  ANCHORAGES: "2026-08-08",
  ...
  // 2026-08-15: the June and September 2027 quick answers carried the old
  // "Reference pricing (2026 Index)" line verbatim.
  BOTTOM_FUNNEL: "2026-08-15",
