...
// 70 -> 63 on 2026-08-30, same night: seven of the eleven new entries are
// also marketed elsewhere, and pending written confirmation George pulled
// them back to drafts. 63 -> 69 on 2026-08-31: confirmation arrived for all
// but one hull, which stays a parked draft permanently off the site.
//
// 69 -> 88 on 2026-09-08: George renewed the motor fleet. Seven motor yachts
// were withdrawn (see lib/retiredYachts.js) and twenty-seven new listings
// were published, twenty-six motor and one sailing monohull. Counted from
// the live dataset with the retired list applied, not from arithmetic.
//
// 2026-09-08, and this is why the paragraphs above read as they do: this
// repository is PUBLIC on GitHub. Comments here are as readable as the site
// itself. Names of the houses this one works alongside, and the names of
// people inside them, were removed for that reason and must not come back:
// George's rule of 7 September is that no other company appears anywhere a
// third party can read it, and a public repo is exactly that.
// 88 -> 100 on 2026-09-14: twelve crewed yachts joined, eight sailing
// catamarans, one power catamaran and three motor yachts, eleven of them
// based in Athens and one in the Ionian. Counted from the live dataset
// after publishing, retired hulls and monohulls excluded.
// 100 -> 91 later on 2026-09-14: George withdrew nine. Three are based
// outside Greece, and six are catamarans whose rate card starts below
// EUR 17,000 a week (see lib/retiredYachts.js). Recounted from the live
// dataset with the retired list applied.
export const FLEET_COUNT = 91;

// ── Walkthrough videos (2026-09-11) ──────────────────────────────────────
// Between 8 and 10 September a cleared walkthrough video went onto the page
// of every yacht whose owner supplied one and whose footage carried no
// third party's name: 65 in the dataset, 61 among the 88 a visitor can
// browse (the four on hidden monohull pages are not counted here). The
// number is printed in the answer units the AI engines lift, so it lives
// here next to FLEET_COUNT and scripts/checkFleetCount.mjs compares it to
// count(video.checked) on every build. Bump it when publish.mjs adds one.
// 61 -> 68 on 2026-09-14: seven of the twelve new yachts arrived with a
// cleared walkthrough, every frame checked for any other company's mark.
// 68 -> 66 the same day, when two of the nine withdrawn yachts took their
// walkthroughs with them.
export const VIDEO_COUNT = 66;
...
export const FLEET_COMPOSITION = {
  // Counted 2026-09-14 from the live dataset, retired hulls excluded:
  // motor 49, power catamarans 16, sailing catamarans 35, monohulls 7.
  // After the nine withdrawals the same evening: motor 46, power
  // catamarans 15, sailing catamarans 30.
  motor: 46,
  powerCat: 15,
  sailingCat: 30,
...
// 45 (42 ως τις 14/9, 51 για λίγες ώρες, 45 μετά τις αποσύρσεις). Ο αριθμός που ζητά η αγορά περισσότερο από κάθε άλλον στο site.
