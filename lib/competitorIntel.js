...
// Phase 7 R37 enrichment: free press-mention check per competitor.
// Uses DuckDuckGo HTML search with site-restriction queries to find
// recent press coverage for each competitor. Free, no API needed.
// Returns array of recent mentions with snippet + source.
export async function checkPressMentions(competitorDomain, competitorLabel) {
  try {
    const q = `"${competitorLabel}" yacht charter`;
    ...
    if (!res.ok) return { mentions: 0, recent: [] };
    const html = await res.text();
    // Capture result hrefs that are NOT the competitor's own domain.
    const re = /<a class="result__url"[^>]*href="([^"]+)"/g;
    const externalDomains = new Set();
    let m;
    while ((m = re.exec(html)) !== null && externalDomains.size < 20) {
      try {
        const u = new URL(m[1]);
        const host = u.hostname.replace(/^www\./, "");
        if (
          !host.includes(competitorDomain) &&
    ...
    return {
      mentions: externalDomains.size,
      domains: Array.from(externalDomains).slice(0, 8),
    };
  } catch {
    return { mentions: 0, domains: [] };
  }
}

// Phase 7 R37 enrichment: domain authority signal via DuckDuckGo
// result count for site-restricted query. Free proxy for "is this
// site indexed and prominent" without Ahrefs API.
export async function checkSiteIndexedSignal(competitorDomain) {
  try {
    const res = await fetch(
      `https://duckduckgo.com/html/?q=site:${competitorDomain}`,
      {
        method: "GET",
        headers: {
          "User-Agent":
    ...
    if (!res.ok) return { indexedSignal: null };
    const html = await res.text();
    // Count result blocks (rough indexed-page count signal)
    const matches = html.match(/<div class="result__body"/g);
    return { indexedSignal: matches ? matches.length : 0 };
  } catch {
    return { indexedSignal: null };
  }
}
...
export async function runCompetitorCheck() {
  ...
  for (const comp of COMPETITORS) {
    try {
      ...
      // Phase 7 R37 - enriched signals (free, no Ahrefs API needed)
      const [pressMentions, siteSignal] = await Promise.allSettled([
        checkPressMentions(comp.domain, comp.label),
        checkSiteIndexedSignal(comp.domain),
      ]);
      const press = pressMentions.status === "fulfilled" ? pressMentions.value : { mentions: 0, domains: [] };
      const indexed = siteSignal.status === "fulfilled" ? siteSignal.value : { indexedSignal: null };
      ...
      summary.competitors.push({
        ...
        pressDomains: press.domains,
        indexedSignal: indexed.indexedSignal,
      });
...
export async function notifyCompetitorDigest(summary) {
  ...
  const lines = summary.competitors.map((c) => {
    ...
    const newLine = c.newUrls.length > 0
      ...
      : "";
    // Phase 7 R37 - enriched signals
    const pressLine = typeof c.pressMentions === "number"
      ? `\n   🗞 Press footprint: ~${c.pressMentions} external domains`
      : "";
    const indexedLine = c.indexedSignal != null
      ? `\n   🔎 Indexed-page signal: ~${c.indexedSignal}`
      : "";
    return `📊 *${c.label}*\n   Sitemap: ${c.totalCurr} URLs (${sign}${c.netChange})${newLine}${pressLine}${indexedLine}`;
  });
