...
// Phase 7 R36 hotfix: use existing lib/kv REST wrapper (no @vercel/kv
// npm dep). Provides the subset of operations we use: get/set + list.
import { kvGet, kvSet, kvLpush, kvLtrim } from "@/lib/kv";

// Shim: tiny adapter so the rest of this file reads like the
// @vercel/kv API I drafted originally.
const kv = {
  get: async (key) => {
...
// €10/month cost cap — never call the Anthropic REST endpoint
...
export async function checkClaude(query) {
  ...
  try {
    ...
    if (err instanceof CostCapExceeded || err instanceof AiFeaturesDisabled) {
      return { engine: "claude", query, skipped: err.message };
    }
