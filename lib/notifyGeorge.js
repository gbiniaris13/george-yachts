...
export async function emailClient({ to, name, brief }) {
  ...
  try {
    ...
    // 2026-08-23 (George's #5): the acknowledgment mirrors the brief
    // back, so the guest sees their request landed whole, and sets
    // out what happens next. Only lines the guest actually gave.
    const briefLines = Array.isArray(brief)
      ? brief.filter((l) => typeof l === "string" && l).map((l) => l.replace(/</g, "&lt;").slice(0, 500))
      : [];
    const briefBlock = briefLines.length
      ? `
        <div style="border-left: 2px solid #7A5C04; padding: 4px 0 4px 18px; margin: 20px 0;">
          <p style="letter-spacing: 0.18em; font-size: 10px; color: #7A5C04; text-transform: uppercase; font-family: Arial, sans-serif; margin: 0 0 8px;">Your brief, as it reached me</p>
          ${briefLines.map((l) => `<p style="font-size: 14px; margin: 2px 0; color: #3a4453;">${l}</p>`).join("")}
        </div>`
      : "";
    const html = `
      <div style="font-family: Georgia, 'Times New Roman', serif; max-width: 560px; margin: 0 auto; color: #1a2433; line-height: 1.7;">
        <p style="letter-spacing: 0.24em; font-size: 11px; color: #7A5C04; text-transform: uppercase; font-family: Arial, sans-serif;">George Yachts Brokerage House</p>
        <p style="font-size: 17px;">Dear ${first},</p>
        <p style="font-size: 16px;">Your enquiry has reached my desk. I read every brief personally, and you will have my reply shortly, usually within two hours.</p>
        ${briefBlock}
        <p style="letter-spacing: 0.18em; font-size: 10px; color: #7A5C04; text-transform: uppercase; font-family: Arial, sans-serif; margin: 24px 0 6px;">What happens next</p>
        <p style="font-size: 15px; margin: 2px 0;">1. I review your brief personally.</p>
        <p style="font-size: 15px; margin: 2px 0;">2. A tailored proposal follows, with yachts, rates and a route that fit what you described.</p>
        <p style="font-size: 15px; margin: 2px 0;">3. You decide at your own pace. No obligation, no follow-up pressure.</p>
        <p style="font-size: 16px; margin-top: 20px;">If your dates are close or anything is urgent, message me directly on WhatsApp: <a href="https://wa.me/17867988798" style="color: #7A5C04;">+1 786 798 8798</a>.</p>
        ...
        <a href="https://georgeyachts.com" style="color: #7A5C04;">georgeyachts.com</a></span></p>
      </div>`;
