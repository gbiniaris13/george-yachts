...
// 2026-06-01 — Brief 06 / G1-B (corrected): FROM on the VERIFIED
// Resend domain = the APEX georgeyachts.com (the newsletter sends
// from it live). The `send.` subdomain is bounce/return-path only,
// NOT a verified sending domain → 403 if used as FROM. Reverted to
// the apex. Consistent with lib/cabin/email.js FROM_DEFAULT.
const FROM_ADDRESS =
  process.env.CABIN_FROM_ADDRESS ||
  "George Yachts · Cabin <cabin@georgeyachts.com>";
...
async function sendEmail({ icon, title, lines, link, cmdUrl }) {
  ...
  const linesHtml = lines
    ...
    .join("");
  const linkHtml = link
    ? `<p style="margin:24px 0 0 0;"><a href="${esc(cmdUrl + link)}" style="color:#DAA110;text-decoration:none;border-bottom:1px solid #DAA110;font-family:Georgia,serif;font-style:italic;">Open in GY Command →</a></p>`
    : "";

  const html = `<!doctype html><html><head><meta charset="utf-8"/></head>
<body style="margin:0;padding:0;background:#F8F5F0;font-family:Helvetica,Arial,sans-serif;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:520px;margin:24px auto;background:#FCFAF4;border:1px solid rgba(218, 161, 16,0.32);border-radius:6px;padding:32px;">
    <tr><td>
      <div style="font-size:10.5px;letter-spacing:3px;text-transform:uppercase;color:#DAA110;font-weight:600;margin-bottom:12px;">The Cabin · Filotimo</div>
