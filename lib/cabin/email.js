...
export async function sendBriefMemberConfirmation({
  ...
  const body = `
    ...
    <p style="margin:0 0 24px 0;">
      <a href="${esc(baseUrl())}/cabin" style="display:inline-block;background:#0D1B2A;color:#F8F5F0;text-decoration:none;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;letter-spacing:2.5px;font-size:12px;font-weight:500;padding:14px 28px;border:1px solid #C9A84C;text-transform:uppercase;">Open your Cabin</a>
    </p>
...
export async function sendBriefSubmittedEmail({
  ...
  if (contributionSections.length > 0) {
    const sectionHtml = contributionSections
      .map((sec) => {
        const voiceHtml = sec.voices
          .map((v) => {
            const bullets = (v.highlights || [])
              ...
              .join("");
            return `
              <div style="margin:0 0 14px 0;padding:10px 14px;background:#FCFAF4;border:1px solid rgba(201,168,76,0.22);border-radius:3px;">
        ...
        return `
          <div style="margin:0 0 18px 0;">
            <div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:2.4px;text-transform:uppercase;color:#C9A84C;font-weight:600;margin-bottom:8px;">${esc(
              sec.sectionTitle,
      ...
      .join("");
    voicesBlock = `
      <div style="margin:24px 0 24px 0;padding:18px 18px 4px;background:rgba(201,168,76,0.05);border-left:2px solid #C9A84C;">
  ...
  if (allergyRollup && typeof allergyRollup === "object") {
      ...
      .map((m) => {
        ...
        const roleTag =
          m.role === "principal_charterer"
            ? ` <span style="font-size:10px;letter-spacing:1.6px;text-transform:uppercase;color:#C9A84C;font-weight:600;">· Principal</span>`
            : "";
        return `
          <li style="margin:6px 0;padding:8px 12px;background:#FCFAF4;border:1px solid rgba(201,168,76,0.18);border-radius:3px;list-style:none;">
    ...
    const empty =
      (allergyRollup.members || []).length === 0
        ...
        : "";
    allergyBlock = `
      <div style="margin:24px 0 24px 0;padding:18px 18px;background:rgba(201,168,76,0.06);border-left:2px solid #C9A84C;">
        <div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;font-size:11px;letter-spacing:2.4px;text-transform:uppercase;color:#C9A84C;font-weight:600;margin-bottom:10px;">
