...
// 2026-06-01 — Brief 06 / G1-B: FROM must be on the VERIFIED Resend
// sending domain. Resend is verified on the `send.georgeyachts.com`
// subdomain (see CLAUDE.md SPF note + the newsletter sender). The
// old default `cabin@georgeyachts.com` is the APEX domain, which is
// NOT a verified Resend sender → Resend 403 "domain not verified" →
// every cabin email silently failed. Default now points at the
// verified subdomain; CABIN_FROM_ADDRESS / RESEND_FROM_ADDRESS env
// still override (but MUST also be on a verified domain if set).
const FROM_DEFAULT =
  process.env.CABIN_FROM_ADDRESS ||
  process.env.RESEND_FROM_ADDRESS ||
  "George Yachts <cabin@send.georgeyachts.com>";
