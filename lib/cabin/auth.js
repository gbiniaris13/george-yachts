...
// 2026-05-22 — George directive: charter clients receive their
// invite weeks (sometimes months) before the voyage. A 14-day
// link forces them to ask for a fresh one if they don't open
// the cabin within a fortnight — friction that breaks the
// boutique-broker feel. 90 days covers the entire pre-charter
// window from booking confirmation through embarkation. Same
// tradeoff as before: if the email is forwarded within the
// window, the recipient gets access, but cabin access is
// already gated by the recipient owning that email account, so
// this is the same threat surface as the email itself.
const OTP_TTL_SECONDS = 90 * 24 * 3600;       // 90 days (~3 months)
...
export async function findMembershipsForEmail(email) {
  ...
  // 2026-05-22 — FK disambiguation. After adding
  // cabins.brief_submitted_by_member_id (→ cabin_members.id), there
  // are now TWO foreign keys between cabin_members and cabins.
  // PostgREST throws "Could not embed because more than one
  // relationship was found" on the generic !inner embed. We pin
  // the join to the cabin_members.cabin_id FK by name. The catch
  // in /api/cabin/auth/request-link silently swallowed this error,
  // returning 200 and zero memberships — that's why CRM "Send
  // invite" lied for the last hour.
  const data = await dbQuery(
    db
      ...
      .select(
        ...
        cabin_id,
        assists_member_id,
        cabin:cabins!cabin_members_cabin_id_fkey!inner (
          id,
          status,
          vessel_name,
...
export async function createMagicLinkOtp(email, targetCabinId = null) {
  ...
  // kvSet returns null on ANY failure (missing env, KV outage) —
  // lib/kv.js swallows errors by design. If the token was never
  // stored, the invite email would still go out and every click
  // would say "expired" forever, while the CRM reports mailed:true.
  // Fail the send instead: the operator sees a real error and can
  // retry, the client never receives a dead link.
  ...
  if (stored === null) {
    throw new Error(
      "kv-write-failed: magic-link token could not be stored (KV down or KV_REST_API_* unset)"
    );
  }
  return otp;
}
...
export async function consumeMagicLinkOtp(otp) {
  ...
  // laptop, phone, iPad etc. for OTP_TTL_SECONDS (90 days). Each
...
export async function createSession({ email, memberships, activeCabinId = null }) {
  ...
  // Same guard as createMagicLinkOtp: a session cookie pointing at a
  // KV row that was never written bounces the guest straight back to
  ...
  if (stored === null) {
    throw new Error("kv-write-failed: session could not be stored");
  }
  return { token, session: value };
}

// -----------------------------------------------------------
// Admin preview session — 2026-05-21
//
// George's request: a "Preview as customer" button in the CRM
// that opens The Cabin in a new tab AS the principal would see
// it, WITHOUT actually logging the admin in as the customer or
// firing any writes (chat / mood-board / consent flips) under
// the customer's identity.
//
// Approach: mint a real session blob in KV (same shape as a
// normal one, so every page/component reads it transparently)
// but flagged `preview_mode: true` with a SHORT TTL (15 min).
// The middleware sees a second `gy_cabin_preview` cookie and
// rejects writes; the layout shows a banner. The principal
// remains the cabin's actual head charterer — what George sees
// is exactly what they would see.
//
// preview_admin_email is recorded so /admin/audit can show "who
// previewed which cabin when", and so any leakage of the
// preview cookie can be traced back to a real admin session.
// -----------------------------------------------------------
const PREVIEW_TTL_SECONDS = 15 * 60; // 15 minutes
export const PREVIEW_COOKIE = "gy_cabin_preview";
...
export async function createPreviewSession({
  ...
  const value = {
    ...
    active_cabin_id: pinValid ? activeCabinId : null,
    issued_at: Date.now(),
    expires: Date.now() + PREVIEW_TTL_SECONDS * 1000,
    // Preview-mode flags — read by layout (banner) and write
    // endpoints (rejection). Never persisted to a long session.
    preview_mode: true,
    preview_admin_email: adminEmail || null,
  };
  await kvSet(`cabin:session:${token}`, JSON.stringify(value), PREVIEW_TTL_SECONDS);
  return { token, session: value, ttl_seconds: PREVIEW_TTL_SECONDS };
}
