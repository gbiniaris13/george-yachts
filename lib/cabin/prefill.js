...
// -----------------------------------------------------------
// Compute aggregate completion % across all VISIBLE sections.
// Mirrors the per-section heuristic in schemas.js + the
// visibility rules in /cabin/brief overview:
//   • little_things — retired from the UI; never counted.
//   • children      — only counted when minors are aboard.
//   • everything else — always counted.
// -----------------------------------------------------------
export async function recomputeCabinCompletion(cabinId) {
  const db = getCabinDb();
  const [rows, manifest, memberRows] = await Promise.all([
    dbQuery(
      db
        .from("cabin_brief_sections")
        .select("completed, section_key")
        .eq("cabin_id", cabinId),
    ),
    dbQuery(
      db
    ...
    // 2026-05-25 — Phase 3: life_aboard writes per-member to
    // cabin_members.personal_details.life_aboard_brief (Angeliki
    // batch 3), so we need this side-table to know if anyone has
    // filled in their Life Aboard. Without it the progress bar
    // would always cap one section below 100% even when every
    // section is genuinely done.
    dbQuery(
      db
        .from("cabin_members")
  ...
  // Visible sections only — matches /cabin/brief overview.
  const visibleKeys = ALL_SECTION_KEYS.filter((k) => {
    if (k === "little_things") return false;
    if (k === "children" && !hasMinors) return false;
    return true;
  });

  const completedFromSections = new Set(
    (rows ?? [])
      .filter((r) => r.completed && visibleKeys.includes(r.section_key))
      .map((r) => r.section_key),
  );

  // Synthesise life_aboard completion from any-member-filled.
  if (visibleKeys.includes("life_aboard") && !completedFromSections.has("life_aboard")) {
    const anyFilled = (memberRows ?? []).some((m) => {
    ...
    if (anyFilled) completedFromSections.add("life_aboard");
  }

  const total = visibleKeys.length || 1;
  const filled = completedFromSections.size;
