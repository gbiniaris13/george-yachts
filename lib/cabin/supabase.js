...
// CABIN_SUPABASE_* comes first and points at the LIVE CRM project
// (ojpcm…). The CRM_SUPABASE_* fallback is the original project the
// Cabin was built on in May 2026 (lqux…) — the CRM migrated off it in
// June but this module kept following the old name, so the guest app
// and the CRM spent the summer on different databases: invites failed
// with no-membership-for-email because the cabin the CRM created
// simply did not exist here. The fallback chain is kept only so local
// dev without the new vars still boots against something.
const SUPABASE_URL =
  process.env.CABIN_SUPABASE_URL ||
  process.env.SUPABASE_URL ||
  process.env.CRM_SUPABASE_URL;
const SUPABASE_SERVICE_ROLE_KEY =
  process.env.CABIN_SUPABASE_SERVICE_KEY ||
