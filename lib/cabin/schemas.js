...
export const healthSchema = z.object({
  ...
  emergency_contact: z
    .object({
      ...
      relationship: optStringShort,
      mobile: optStringShort,
      // 2026-09-02 — no format validation. The old
      // z.string().email() failed the WHOLE health payload on one
      // malformed address ("bob@aol,com"), so the name and phone
      // typed alongside it silently never saved and the section
      // could never complete — the exact hard-reject the house rule
      // forbids. A slightly wrong emergency email is still readable
      // by a human; a vanished emergency contact is not.
      email: optStringShort,
    })
    .optional(),
});
...
export const diningSchema = z.object({
  ...
  // 2026-05-24 — Christos pass: free-text "other" capture after
  // each multiple-choice. Member can name a style we didn't list.
  breakfast_styles_other: optString,
