...
export const arrivalSchema = z.object({
  ...
  // line at the top of section 01. It's the person already on the
  // ground in Greece — a relative, a personal concierge, a hotel
  // representative — that the captain can coordinate with in case
  // of last-minute changes (rescheduled flight, marina move, etc.)
  // without having to wake the principal.
  //
  // Default state: same_as_principal = true. The principal IS the
  // local contact unless they nominate someone else. If they tick
  // "someone else", name + mobile become required-ish (still
  // optional fields — captain just falls back to the principal).
  local_contact: z
    .object({
      same_as_principal: optBoolRadio,
